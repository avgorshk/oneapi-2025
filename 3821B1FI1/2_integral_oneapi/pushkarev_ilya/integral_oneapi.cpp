...
float IntegralONEAPI(float start, float end, int count, sycl::device device) {
    ...
    try {
        ...
        const int wg_size = 8;
        const int rounded = ((max_count + wg_size - 1) / wg_size) * wg_size;
        const int num_groups = rounded / wg_size;
        ...
        q.submit([&](sycl::handler& h) {
            ...
            sycl::accessor<float, 1, sycl::access::mode::read_write,
                sycl::access::target::local> local(sycl::range<1>(wg_size * wg_size), h);

            h.parallel_for(sycl::nd_range<2>(
                {rounded, rounded}, {wg_size, wg_size}), [=](sycl::nd_item<2> item) {
        ...
        result = result_buf.get_access<sycl::access::mode::read>()[0];
