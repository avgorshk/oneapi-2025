...
std::vector<float> JacobiAccONEAPI(const std::vector<float> a,
                                   ...
                                   sycl::device device) {
  std::vector<float> cur_ans(b.size(), 0.0f);
...

  {
    ...
    sycl::buffer<float, 1> buf_cur(cur_ans.data(), cur_ans.size());
    ...
    while (step++ < ITERATIONS) {
      ...
      queue.submit([&](sycl::handler &cgh) {
        ...
        auto in_cur = buf_cur.get_access<sycl::access::mode::read_write>(cgh);
        ...
        cgh.parallel_for(sycl::range<1>(size), reduction,
                         [=](sycl::id<1> id, auto &error) {
                           ...
                           for (int j = 0; j < size; j++) {
                             if (i != j) {
                               cur -= in_a[i * size + j] * in_prev[j];
                             }
                           }
                           cur /= in_a[i * size + i];
                           in_cur[i] = cur;

                           float diff = sycl::fabs(cur - in_prev[i]);
                           error.combine(diff);
                         });
      });
      queue.wait();
      ...
      {
        auto host_cur = buf_cur.get_host_access();
        auto host_prev = buf_prev.get_host_access();
        for (int i = 0; i < size; i++)
          host_prev[i] = host_cur[i];
      }
    }
  }
  return cur_ans;
