...
std::vector<float> JacobiAccONEAPI(const std::vector<float> a,
                                   ...
                                   sycl::device device) {
  std::vector<float> curr_res(b.size(), 0.0f);
  std::vector<float> prev_res(b.size(), 0.0f);
  int size = b.size();
  int step = 0;
  float error = 0.0f;

  {
    ...
    sycl::buffer<float, 1> buf_curr(curr_res.data(), curr_res.size());
    sycl::buffer<float, 1> buf_prev(prev_res.data(), prev_res.size());
