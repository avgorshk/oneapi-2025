...
std::vector<float> JacobiSharedONEAPI(const std::vector<float> a,
  ...
  float *shared_cur = sycl::malloc_shared<float>(size, queue);
  ...
  queue.memset(shared_cur, 0, sizeof(float) * size);
  ...
  while (step++ < ITERATIONS) {
    ...
    queue.parallel_for(sycl::range<1>(size), reduction,
                       [=](sycl::id<1> id, auto &error) {
                         int i = id.get(0);
                         float cur = shared_b[i];
                         for (int j = 0; j < size; j++) {
                           if (i != j) {
                             cur -= shared_a[i * size + j] * shared_prev[j];
                           }
                         }
                         cur /= shared_a[i * size + i];
                         shared_cur[i] = cur;

                         float diff = sycl::fabs(cur - shared_prev[i]);
                         error.combine(diff);
                       });
    queue.wait();
    ...
    *shared_error = 0;
    ...
    queue.memcpy(shared_prev, shared_cur, size * sizeof(float)).wait();
  }
  queue.memcpy(ans.data(), shared_cur, size * sizeof(float)).wait();

  sycl::free(shared_a, queue);
  sycl::free(shared_b, queue);
  sycl::free(shared_cur, queue);
